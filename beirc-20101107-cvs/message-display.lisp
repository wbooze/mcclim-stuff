...
(defun invoke-formatting-message (stream message receiver preamble-writer message-body-writer)
  (let* ((*current-message* message)
	  ...
	  (size (bounding-rectangle-size (sheet-parent stream*))))
                   ...
                   (with-drawing-options (stream* :ink +gray+)
                     (if (and *meme-log-bot-nick*
                         ...
                         (with-output-as-presentation (stream* (make-meme-url message) 'meme-url)
                           (format-timestamp message))
                         (format-timestamp message)))))))
      (updating-output (stream* :cache-value size :cache-test #'eql)
	(let* ((colors
		 (list
		   +green+ +green2+ 
		   +red2+ 
		   ...
		   +royal-blue+ 
		   +pink+ +pink2+ 
		   +burlywood+ +burlywood2+ +burlywood3+ 
		   +olivedrab+ +olivedrab2+ +olivedrab3+ 
		   +wheat+ +wheat2+ +wheat3+
		   +seashell+ +seashell2+ 
		   +cornsilk+ +cornsilk2+ 
		   +goldenrod+ +goldenrod2+ 
		   +darkgoldenrod1+ +darkgoldenrod2+))
		(my-color (funcall (lambda () (elt colors (random (length colors)))))))
	  (formatting-row (stream*)
	    ;; this is the place to change the nicknames appearance!
	    (if (eql *timestamp-column-orientation* :left)
	      (with-drawing-options (stream* :ink my-color :text-size :very-large)
		(formatting-cell (stream* :align-x :left :min-width `(1 :character))
		  (output-timestamp-column :left))
		(formatting-cell (stream* :align-x :left :min-width `(3 :character))
...
(defmacro do-colored-string ((string-var str) &body body)
  `(dolist (part (split-before *color-scanner* ,str))
     (multiple-value-bind (message foreground background)
       ...
       (with-drawing-options (*standard-output* :text-size 19) ;;this is the place to change message-body text-sizes
...
(defun beirc-app-display (*application-frame* *standard-output* receiver)
  (let* ((messages (and receiver (messages receiver)))
         (*max-preamble-length* (loop for message in messages
                                      minimize (preamble-length message))))
    (formatting-table (t)
