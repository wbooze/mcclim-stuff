...
(unless (sb-thread:mutex-value *permanent-queue*)
  (sb-sys:without-interrupts (sb-sys:allow-with-interrupts (sb-thread:grab-mutex *permanent-queue* :waitp nil))))
...
(defun disable-process (process)
  (sb-thread:interrupt-thread
   ...
   (lambda ()
     (catch 'interrupted-wait (sb-sys:without-interrupts (sb-sys:allow-with-interrupts (sb-thread:grab-mutex *permanent-queue*)))))))
...
(defmacro with-lock-held ((place &optional state) &body body)
  (let ((old-state (gensym "OLD-STATE")))
    `(let (,old-state)
      (unwind-protect
           (progn
             (sb-sys:without-interrupts (sb-sys:allow-with-interrupts (sb-thread:grab-mutex ,place)))
	     (when ,state
        ...
        (sb-sys:without-interrupts (sb-thread::release-mutex ,place))))))
